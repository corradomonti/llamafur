...
import utils.webgraph.IntMapGraph;
...
public class WikipediaCategoryProducer {
	...
	private static final int ESITMATED_NUM_OF_PAGES = 14226207; // this is needed only to log progress 




	private final WikipediaDocumentSequence wikipediaDocumentSequence;
	...
	WikipediaCategoryProducer(WikipediaDocumentSequence wds) {
	...
	private void saveAllTo(String basename) throws Exception {
		...
		do {
			try {
				SerializationUtils.save(pageId2Name, basename + "pageId2Name.ser");
				SerializationUtils.save(catName2Id, basename + "catName2Id.ser");
				SerializationUtils.save(page2cat, basename + "page2cat.ser");
				SerializationUtils.save(MapUtils.invert(catName2Id), basename + "catId2Name.ser");
				SerializationUtils.save(MapUtils.invert(pageId2Name), basename + "pageName2Id.ser");
				...
				e.printStackTrace();
				basename = askForString("The selected basename (" + basename + ")" +
